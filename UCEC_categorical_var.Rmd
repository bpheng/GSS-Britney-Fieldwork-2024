...
ucec_all <- readRDS("data/ucec_cancer.rds")

## Only the top 15 validated RAVs
sampleScore_sub <- read.csv("data/ucec_cancer_sampleScore.csv", row.names = 1)

## UCEC categorical variables
charTb <- read.csv("data/ucec_cancer_char.csv", row.names = 1)
```
...
ztable(wtest_ucec_sig_wvalue) %>%
  ...
  print(caption='All Sample W-test with p-values < 0.01')
```
